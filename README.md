...
- base/current 可以使用同一个工程目录；两侧身份由各自确认后的分支、tag 或 commit 决定。Skill 会固定到具体 commit，并在独立 worktree 中分析，不会切换你的当前工作区。
- 直接产物模式会先解析 JAR；只有依赖坐标仍缺失时才使用对应侧分支补全。分支名不精确时，Skill 会在本地分支和已经存在的远端跟踪分支中查找；只有唯一 commit 才会自动采用。多个不同 commit 候选会在 Maven 执行前要求确认，不会自动 `git fetch` 或猜测。
