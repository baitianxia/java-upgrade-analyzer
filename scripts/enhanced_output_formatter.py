...
REASON_CODE_EXPLANATIONS = {
    ...
    'REFLECTION_CLASS_LOOKUP': {
        'reason': '已追踪到使用精确类名的反射加载路径，但尚不能静态证明该路径会在当前运行条件下成功触发',
        'action': '核对反射入口的启用条件，并通过启动或集成测试确认目标类是否实际加载'
    },
...
def _alert_rows_for_result(result):
    ...
    for detail in details:
        ...
        rows.append({
            ...
            'compile_impact': getattr(result, 'compile_impact', '') or '',
            'runtime_link_impact': getattr(result, 'runtime_link_impact', '') or '',
