...
def _markdown_heading_fragment(line):
    """Return the native Markdown fragment used by common report viewers."""
...
def _dependency_current_calls_cell(row):
    api_count = int(row.get("confirmed_api_count") or 0)
    relationships = int(row.get("confirmed_relationship_count") or 0)
    ...
    if incomplete:
        ...
        lines.extend([
            ...
            (
                ...
                "相同范围的结构化数据见"
                "[依赖分析 CSV](all-affected-dependencies.csv)"
...
def render_api_and_calls(findings, api_model=None):
    ...
    if completed:
        ...
        lines.extend([
            ...
            "",
            (
                "本节按依赖坐标分组，展示排序靠前的 `reachable` 和 `uncertain` API。"
        ...
        for index, (coord, dependency_rows) in enumerate(
            ...
            lines.extend([
                ...
                "",
                (
                    f"本依赖展示 {reachable_count} 个 `reachable`、"
...
def _full_relationship_cell(row, alert_details):
    ...
    if conclusion == "已确认影响":
        ...
        statuses = ("uncertain", "not_analyzed")
        prefix = "候选关系："
    else:
        statuses = ()
        prefix = ""
...
_MAIN_REPORT_TOC_ENTRIES = (
    ...
    ("### 未完成分析的依赖", "未完成分析的依赖", 1),
    ("### 已完成分析的依赖", "已完成分析的依赖", 1),
    ("## 二、API 及调用关系", "二、API 及调用关系", 0),
    ("### 未完成分析的 API", "未完成分析的 API", 1),
    (
        "### 已确认触达与结论未确定的 API",
        "已确认触达与结论未确定的 API",
        1,
    ),
    ("### 其他已完成状态统计", "其他已完成状态统计", 1),
    ("## 三、用户可见文件说明", "三、用户可见文件说明", 0),
)
