...
`--final-artifact` 已能绑定最终制品路径与 SHA-256，但外部提供的 JAR 不能自动证明其对应当前源码 revision。只有分析器亲自构建的制品，或经过固定 Git revision 与制品 SHA 双重校验的清单，才能将源码与制品标记为 `aligned`。

完成标准：

- 内部构建路径自动记录 revision、目标模块、构建命令、profile 和制品 SHA；
- 外部制品默认保持 `unverified`，不得仅凭文件名或 Maven 版本推断源码一致；
- 固定清单同时验证源码 revision 与制品 SHA 后，Step5 能消费同一份对齐证据；
- 污染工作区、错模块、旧制品和 revision 不一致均有反例回归。
