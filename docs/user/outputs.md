...
主报告的内容顺序：标题、生成信息以及源码或分析范围说明之后先提供可点击目录，再进入正文。目录只列出
本次实际生成的章节，使用 Markdown 标题自身的锚点，不插入 HTML 标签；完整文件入口放在
对应正文段落和末尾文件说明中。
...
| `.runtime/observability/progress.previous.jsonl` | `progress.jsonl` 的上一轮转段，最多保留一个；当前段与上一段各自最多 8 MiB，单事件最多 256 KiB |
...
进度事件超出单事件上限时只截断观测字段并记录截断事实；轮转、截断或非权威进度文件写入失败
不能让正式分析失败，也不能改变结论。
...
Step6 在主报告中按依赖坐标分组，限量展示排序靠前的“已确认影响”和“结论未确定”API；依赖之间按整体影响程度排序，同一依赖内再按结论状态、优先级分数和调用证据排序。依赖和 API 的主报告明细都必须设置展示上限，并标明展示数、总数和未展示数；对应位置必须直接给出完整 Markdown 与 CSV 的入口及用途。“已确认不受影响”和“静态分析未找到路径”只在主报告给出统计数量，不能据此推断安全；所有状态的逐项记录仍进入完整 Markdown 和同数据、同排序的 CSV。顶部目录只承担章节导航，底部文件说明继续作为完整交付索引。部分分析时，选择前全集只保留在原始变化清单和范围记录中。

### 主报告固定内容顺序

`deliverables/report.md` 必须先给出标题、生成信息、必要的范围说明和章节目录，再按以下顺序组织；
目录不能混入产物清单或完整明细链接：
