...
使用统一入口的 `--step auto` 时，流程会自动运行到下一个必要确认点。Step0 始终用一张 Base/Current 表统一确认最终制品、版本分支、目标模块、构建工具、JDK 目录、必填应用源码和可选依赖包源码；自动识别值也只在这里确认一次。Step1 仅在依赖身份或已提供依赖包源码的仓库/不同 commit 存在真实歧义时汇总询问；Step2 不设置固定确认。依赖 API 变化识别出至少两个候选依赖时保留全量/部分范围选择；0 个或 1 个候选不存在实际范围取舍，系统直接继续。确认范围后，系统触达分析和最终报告连续完成。
...
主报告的内容顺序：标题、生成信息以及源码或分析范围说明之后先提供可点击目录，再进入依赖层面结论。目录只列出本次实际生成的章节，不会链接到不存在的条件章节；跳转使用 Markdown 标题自身的锚点，不在人工报告中插入 HTML 标签。
...
应用源码在 Step0 必须提供并确认；当前 Git 仓库即使被自动识别，也不能跳过该确认。依赖包源码可在同一张 Step0 表格中提供或留空。无论依赖包源码是否提供，最终依赖身份、变化事实和可执行边仍以最终二进制制品为准。
...
Step6 在主报告中按依赖坐标分组，限量展示排序靠前的“已确认影响”和“结论未确定”API；依赖之间按整体影响程度排序，同一依赖内再按结论状态、优先级分数和调用证据排序。依赖、API 和运行时资源的主报告明细都必须设置展示上限，并标明展示数、总数和未展示数；对应位置必须直接给出完整 Markdown 和 CSV 或证据文件入口及用途。“已确认不受影响”和“静态分析未找到路径”只在主报告给出统计数量，不能据此推断安全；所有状态的逐项记录仍进入完整 Markdown 和同数据、同排序的 CSV。顶部目录只承担章节导航，底部原有文件说明继续作为完整交付索引。部分分析时，选择前全集只保留在原始变化清单和范围记录中。
...
主报告先回答“发生了什么、证据证明到哪里、哪些事实仍未确认”，只使用前文定义的人工结论，不展示分析引擎内部状态。具体行动说明必须受当前证据边界约束，不得把静态未命中表述为安全，也不得生成发布判断、修改判断、具体实现方案或验证完成结论。
