...
1. 按[所有缺陷的根因与系统性闭环](testing-strategy.md#71-所有缺陷的根因与系统性闭环)记录实际错误表现、正确行为、完整因果链和最早偏差点；根因未确认时不得进入正式修复；
2. 明确分类为架构/设计、实现/代码、配置/环境或需求/使用问题，并给出“个例或普遍问题”的范围结论及证据；
3. 以根因机制全面排查同一抽象和全部相关实现、调用方、步骤、平台及上下游路径，记录搜索方法、同类命中点和逐项处置，禁止只修反馈位置；
4. 记录上一轮逃逸原因和系统性修复范围；设计问题先修正文档拥有的模型/边界，代码问题修复最早公共实现点；
5. 至少绑定一个只走公开入口的精确黑盒方法和一份 `system_generated=false`、由两个独立机制交叉产生的真值；
6. 真值必须包含可解析且非空的反例或邻近控制值，回归还必须覆盖原始复现及横向排查发现的其他同类位置；
7. 同时绑定必要的白盒/性能回归及它们应被哪些 profile 实际选中；
8. 至少一个回归位于合入前 profile。测试改名、删除、真值被系统输出替换、反例丢失或 profile 声明与实际 selector 不符都会由 `defect_regression_gate.py` 阻断；
9. 同一根因族再次逃逸时必须重新审视模型或架构并扩大同类排查范围，不能继续追加同类案例补丁。
...
性能门必须同时记录输入规模、冷/热 cache、总耗时、阶段耗时、P50/P95、CPU 秒、平均核数和可取得的峰值内存；门禁从原始样本复算分位数与平均核数。固定性能 fixture 位于 `tests/fixtures/binary_first/performance_gate.json`，其内容身份在 support manifest 中固定。当前记录于 2026-08-28 在固定 macOS arm64 参考环境完成候选采集、provisional 绑定、全新目录独立完整复采、final build 和正式回放，并含 CPU 原始证据。大规模 fact-store 门与两条冷启动完整流水线门都覆盖 400 JAR/100000 class：一条比较完全相同的两侧，另一条确定性替换 current 侧的一个 JAR并校验 250 条实现变化。正式复采 cold 为 115.307 秒，三次 warm 的 P50/P95 为 43.645/43.726 秒；相同两侧完整流水线为 93.932 秒且正式结果/validation issue 均为 0，单 JAR 变化流水线为 125.512 秒且变化事实/正式结果均精确为 250。changed 路径相对上一份独立复采的 266.617 秒下降 52.924%，source-owned 上限固定为 133.308 秒；新记录的 lifecycle peak RSS 为 750616576 字节，pipeline-reported peak 为 627195904 字节。完整门继续覆盖 runtime reconciliation、trace、generation 和独立 Oracle，从两侧 SQLite 与已落盘 Oracle 结果读取实际类数、变化数量与种类、正式结果状态和问题数，并逐阶段记录累计峰值 RSS、单独限制完整流水线 RSS，避免配置中的理论规模掩盖事实丢失，也避免缩小样本掩盖超线性协调、全表物化、双侧对象重叠或逐 class 子进程退化。任何新结果缺少 CPU 证据、超过 50% 改善硬线或派生关系不一致时失败；历史记录若确实没有 CPU 原始数据只能显式声明，不能补造。
