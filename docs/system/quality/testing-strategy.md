...
- 源码构建型制品还必须固定构建 JDK major；guard 用该 JDK 同时绑定 `JAVA_HOME` 和 `PATH` 后才允许构建，JDK 不符时在构建前失败关闭；
...
当前提交的 2026-08-28 规模证据通过“候选采集 → provisional 绑定 → 全新目录独立完整复采 → final 证据构建 → 当前实现回放”形成，保留三次 warm 原始 wall/CPU 样本、两条完整流水线的 CPU/RSS/阶段数据和总计派生值。复采 cold 为 115.307 秒，warm P50/P95 为 43.645/43.726 秒；相同两侧完整流水线为 93.932 秒且正式结果/validation issue 均为 0，单 JAR 中 250 class 变化流水线为 125.512 秒且变化事实/正式结果均精确为 250。changed 路径相对上一份 266.617 秒记录提升 52.924%，CPU 从 852.204 降到 269.239 秒；lifecycle peak RSS 从 758317056 降到 750616576 字节，pipeline-reported peak 降到 627195904 字节。性能 policy 将相同 source-bound workload 的 changed 上限固定为 133.308 秒，不能用放宽 fixture 或减少事实量通过。所有 smoke、scheduled benchmark 和 release scale 都必须输出 CPU 秒与平均核数，缺失或派生关系错误即失败；历史记录没有 CPU 原始数据时只能明确标记缺口，禁止补造数字。
...
测试清单持续变化，权威数量只来自当次 `test_suite_runner.py` JSON。2026-08-28 的本地 discovery/Release 快照为 3508 项（黑盒 61、白盒 3281、性能 166）；这只是本次变更的审计快照，不是后续固定目标，也没有在当前主机上证明 Windows 原生通过。Windows 投影只精确选择可在原生 Windows 执行的方法，不得把明确的 Unix-only 测试带入后再用 skip 冒充通过。完整白盒只允许两项精确登记的替代执行：一个由 Release 真实项目门运行，一个由 Windows 原生矩阵运行；其他任何 skip 都失败。测试健康门另验证登记的关键分支、变异和稳定性复跑；具体命中数以当次门禁 JSON 为准。数字只用于发现测试被漏选或误分类，不作为覆盖充分性的替代指标。

同日从最终工作树执行的汇总 `release` 重新 discovery 并完整通过 3508/3508 项（61 blackbox、3281 whitebox、166 performance；3506 项实际通过、2 项精确替代执行 skip），0 failure、0 error、0 expected failure、0 unexpected success、0 loader failure、0 非预期 skip。测试有效性门另得到 98/98 个登记分支替代、15/15 个登记变异被杀死，以及 84 项健康集连续两轮身份和时序稳定。6/6 个固定真实项目通过，各项目 issue 列表为空；三个 Spring source-build 项目由 manifest 固定的 JDK 21 构建，错误 JDK 会在 clone/构建前失败；完全相同的 MyBatis XML 双侧另由独立 33 制品完整字节身份 Oracle 证明正式结果严格为空。400 JAR/100000 class 的 source-bound 记录证据重放通过，250 个输入变化、250 个正式结果、0 validation issue，全流程无子进程超时。Release 已实际执行 MyBatis 真实项目替代项，Windows GUI 项仍等待原生矩阵。当前结果仍不证明 Windows 六个原生 cell、目标 Windows 10/Xeon/32GB 的 27GB Step4 ≤5h 或远端 GitHub required-check 配置已经通过/启用；这些结论必须等待对应实机和仓库设置证据。
