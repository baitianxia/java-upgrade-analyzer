...
10. **用户输出先讲结论与原因。** `report.md` 固定按“依赖层面结论 → API 及调用关系 → 用户可见文件说明”排列；全量依赖结果在 `all-affected-dependencies.md`，全量 API 及完整调用关系在 `all-impact-details.md`，`alerts.csv` 保留原始分析记录。三者用途必须分别说明，内部术语不能替代用户可理解的表达。
