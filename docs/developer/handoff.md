...
10. **用户输出先讲结论与原因。** `report.md` 固定按“依赖层面结论 → API 及调用关系 → 用户可见文件说明”排列；本轮范围内全部依赖结果在 `all-affected-dependencies.md/.csv`，全部 API 及完整调用关系在 `all-impact-details.md/.csv`，同类 Markdown 与 CSV 使用相同数据和排序，`alerts.csv` 保留原始分析记录。各类文件用途必须分别说明，内部术语不能替代用户可理解的表达。
