...
当前共 6 项：已完成 0、正在执行 0、待执行 6、阻塞 0、失败 0。每项均未完成交付验证；
验收条件、依赖关系和现状证据见 [TODO](../../TODO.md)。

1. 建立当前 HEAD 的完整质量基线。
2. 明确 Kotlin 与混合语言能力边界。
...
定向回归、`quick`、`smoke_core` 和 `smoke_step5` 已通过；完整 456 项 Step5 模块有
455 项通过，唯一错误仍是已回退旧测试调用不存在的 `_build_direct_usage_result`，不得表述为
完整 Step5 profile 已通过。
...
`smoke_core`、`smoke_step5` 与用户场景已通过；完整 Step5 profile 仍受已回退的旧测试 helper
调用错误阻断，不能表述为当前 HEAD 全门禁通过。
