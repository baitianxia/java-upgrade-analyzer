...
源码辅助解析的两条路径使用相同的绝对文件身份，并在词法预处理时正确移除 Java 文本块，
避免文本内容伪造调用。类型推断支持泛型 cast；方法调用前的显式类型参数（例如
`receiver.<Type>call()`）在判断 receiver 和参数表达式前先屏蔽，不能误识别为比较表达式。
无 import 的 `java.lang` 常用类型（包括包装类型、`CharSequence`、`Iterable`、`Enum`、
`Record` 等）按隐式导入解析，但不能把任意同名工程类型提升为这些平台类型。
