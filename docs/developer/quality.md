...
- 依赖层和 API 层必须统一使用“变化总数、已完成分析、未完成分析、确认影响、确认不受影响、尚未确认影响”六列；总数必须等于已完成与未完成之和；依赖已有确认影响但仍有其他 API 未完成时，必须同时保留确认影响事实和未完成状态；
...
- 全量依赖结果进入 `all-affected-dependencies.md`，全量 API 结果及完整调用关系进入 `all-impact-details.md`，两者不得合并为同一个链接；
