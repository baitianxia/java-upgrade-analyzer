...
真实项目或用户使用中发现的缺陷必须先视为“现有策略失效的证据”，而不只是待修的一行代码。修复同一个变更必须完成以下闭环：

1. 保存能稳定复现错误的最小公开输入和实际错误输出；
2. 在运行修复后系统之前，由 JVM/OpenJDK、独立解析器、规范或经记录的人工复核写出正确结果及近邻反例；
3. 说明根因族和上一轮未发现它的具体原因，例如输入维度缺失、Oracle 盲区、断言过弱、测试未进入 PR 门禁、skip 或平台缺失；
4. 用黑盒测试保护外部语义，用白盒测试保护当前根因；涉及并发、规模或平台时再加入性能或原生平台回归；
5. 在 `escaped_defect_regressions.json` 登记每个精确 selector 及其必跑 profile；运行 `defect_regression_gate.py`；
6. 从质量 JSON 核对该 profile 的实际 run、failure、error、skip，而不是只确认测试文件存在；
7. 同根因族再次出现时重新打开模型/架构评审，不接受新的局部 `if` 或只针对当前项目的 fixture 补丁。
