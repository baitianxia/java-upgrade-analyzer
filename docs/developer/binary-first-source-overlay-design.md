...
源码覆盖由实际可用输入决定，不是授权开关。编译模式已经取得被分析系统源码，系统必须直接使用，不能因为用户没有补充依赖源码而关闭。Step2 只负责说明源码可以增加文件/行号、声明与注解、可读上下文和候选关系，并在受支持的常量内联场景与字节码共同形成证明，同时说明它不能覆盖二进制裁决；随后通过统一入口收集用户还能提供的源码位置，或记录暂不补充。直接制品模式的同一入口可以同时接收被分析系统源码和依赖源码。用户不需要预先区分类型，系统按项目范围和依赖坐标分类；只有无法可靠分类时才请求归属映射。
