...
class UserVisibleOutputContractTest(unittest.TestCase):
    ...
    def test_outputs_doc_uses_reader_facing_conclusion_terms(self):
        ...
        self.assertIn("仍不确定", reading_section)
        self.assertIn("本次未完成分析", text)
        self.assertIn("not_found_in_static_analysis", text)
        ...
        self.assertNotIn("确认有影响", text)
        self.assertNotIn("确认不受影响", text)
        self.assertNotIn("not_impacted", text)
        self.assertNotIn("analyzer_edges.csv", text)
        self.assertNotIn("JApiCmp", text)
    ...
    def test_outputs_doc_separates_human_first_files_from_program_files(self):
        ...
        step5_text = text[text.index("## Step5：系统触达证据") : text.index("## Step6：最终报告")]
        self.assertLess(step5_text.index("`evidence/call_chain/alerts.csv`"), step5_text.index("`evidence/call_chain/summary.json`"))
        self.assertIn("`.runtime/indexes/s5_query_index.json`", text)
